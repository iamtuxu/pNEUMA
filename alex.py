...
def create_new_trajectory_data_frame():
    headings_names = ['track_id', 'type', 'entry_gate', 'entry_time', 'exit_gate', 'exit_time', 'traveled_distance',
                      'avg_speed', 'lat', 'lon', 'speed', 'tan_accel', 'lat_accel', 'time']
...
start = time.time()
name = 'aka'
path_input_trajectory_data = "/Users/txu81/Desktop/epfl/2FD/tuxu_dataset_25fps.csv"  # csv file
path_to_export = "/Users/txu81/Desktop/test/"  # folder
...
for tracked_vehicle_id in range(1, len(trajectory_data_array)):
    ...
    for j in range(8, len(trajectory_data_array[tracked_vehicle_id]), 6):
        try:
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8]] = float(
                trajectory_data_array[tracked_vehicle_id][j])  # 8: Latitude [deg]
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8 + 1]] = float(
                trajectory_data_array[tracked_vehicle_id][j + 1])  # 9: Longitude [deg]
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8 + 2]] = float(
                trajectory_data_array[tracked_vehicle_id][j + 2])  # 10: Speed [km/h]
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8 + 3]] = float(
                trajectory_data_array[tracked_vehicle_id][j + 3])  # 11: Tan. Accel. [ms-2]
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8 + 4]] = float(
                trajectory_data_array[tracked_vehicle_id][j + 4])  # 12: Lat. Accel. [ms-2]
            new_trajectory.at[divmod(j, 6)[0] - 1, column_names[8 + 5]] = float(
                trajectory_data_array[tracked_vehicle_id][j + 5])  # 13: Time [ms]
        except IndexError:
            continue
        except ValueError:
            continue
    ...
    for row in range(0, new_trajectory.shape[0]):
        fwd_azimuth, back_azimuth, distance = geodesic.inv(starting_lon, starting_lat,
                                                           float(new_trajectory['lon'][row]),
                                                           float(new_trajectory['lat'][row]))
        new_trajectory.loc[row, 'distance'] = distance
        ### calculate Lane number
        if float(new_trajectory['lon'][row]) < -6.238846886 * float(new_trajectory['lat'][row]) + 260.7563820:
            lane = 1
        elif float(new_trajectory['lon'][row]) < -6.228387442 * float(new_trajectory['lat'][row]) + 260.3591981:
            lane = 2
        elif float(new_trajectory['lon'][row]) < -6.325706595 * float(new_trajectory['lat'][row]) + 264.0566965:
            lane = 3
        else:
            lane = 4
        new_trajectory.loc[row, 'lane'] = lane
    new_trajectory['Elevation'] = velev(lon=new_trajectory['lon'].to_numpy(), lat=new_trajectory['lat'].to_numpy(),
...
print()
print()
print('Time for extracting ' + str(tracked_vehicle_id) + ' vehicles was ' + str(
    int(divmod(end - start, 60)[0])) + ' minutes and ' +
